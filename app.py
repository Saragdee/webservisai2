...
def create_supplier():
    data = request.get_json()
    cur.execute('INSERT INTO suppliers (name, email, phone) VALUES (%s, %s, %s) RETURNING id',
                (data['name'], data['email'], data['phone']))
    conn.commit()
    new_id = cur.fetchone()[0]
    return '', 201, {'Location': url_for('get_supplier', id=new_id)}
...
def update_supplier(id):
    ...
    cur.execute('UPDATE suppliers SET name=%s, email=%s, phone=%s WHERE id=%s',
                (data['name'], data['email'], data['phone'], id))
    conn.commit()
    return '', 200, {'Location': url_for('get_supplier', id=id)}
...
def create_product():
    data = request.get_json()
    cur.execute('INSERT INTO products (name, price, supplier_id) VALUES (%s, %s, %s) RETURNING id',
                (data['name'], data['price'], data['supplier_id']))
    conn.commit()
    new_id = cur.fetchone()[0]
    cur.execute('SELECT * FROM products WHERE id=%s', (new_id,))
    new_product = cur.fetchone()
    return jsonify(new_product), 201, {'Location': url_for('get_product', id=new_id)}
...
def update_product(id):
    ...
    cur.execute('UPDATE products SET name=%s, price=%s, supplier_id=%s WHERE id=%s',
                (data['name'], data['price'], data['supplier_id'], id))
    conn.commit()
    return '', 200, {'Location': url_for('get_product', id=id)}
